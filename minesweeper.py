...
class Minesweeper:
    ...
    def __init__(self, tk):
        ...
        self.images = {
            ...
            "quantumflag": PhotoImage(file = "images/original/tile_q.png"),
            "wrong": PhotoImage(file = "images/tile_wrong.gif"),
            "numbers": []
        }
        self.images["numbers"].append(PhotoImage(file = "images/original/tile_05.png"))
        for i in range(2, 9):
            self.images["numbers"].append(PhotoImage(file = "images/original/tile_"+str(i*5)+".png"))
    ...
    def onClick(self, tile):
        ...
        if tile["isMine"] == True:
            if random.uniform(0.0, 1.0) <= 0.5: #simulates coin toss/measurement
                self.gameOver(False) #end game
                return
            else:
                #continue/update mine count
                self.mines -= 0.5
                tkMessageBox.showinfo('You survived!', 'This square had a half chance of detonating, but u were lucky this time!')
        ...
        if tile["mines"] == 0:
            ...
            tile["button"].config(image = self.images["numbers"][int(tile["mines"]*2)-1])
    ...
    def onRightClick(self, tile):
        ...
        #Implementing random chance for Quantum flag
        #isQuantum = False
        ...
        if tile["state"] == STATE_DEFAULT:
            ...
            # if a mine
            if tile["isMine"] == True:
                self.correctFlagCount += 0.5
            self.flagCount += 0.5
            self.refreshLabels()
        #if tile["state"] == STATE_DEFAULT  and isQuantum == True:
         #   tile["button"].config(image = self.images["quantumflag"])
          #  tile["state"] = STATE_FLAGGED
           # tile["button"].unbind(BTN_CLICK)
            #if a mine
            #if tile["isMine"] == True:
            ...
            #neighbors = self.getNeighbors(tile.get('x'),tile.get('y'))
            #array_size = neighbors.len(array)
            #neighbor = random.uniform(0, array_size)
            #neighbor = int(random.uniform)
            #neighbor.clearTile()
